...
import {getValueByKeys, isExternalLink} from "@/utils/common";
import  {toSysViewComponentPath} from "@/router";
import Iframe from "@/views/iframe.vue";
import layout from "@/layout/layout.vue";
import {Router} from "vue-router";
/**
 * 合并本地路由和服务端菜单，追加isIframe和isNewPage参数到meta内
 * @param serverRoutes
 * @param sysRouteMap
 * @param matched
 */
export const mergeServerRoute = (
  ...
  serverRoutes.forEach((x: IServerMenus) => {
    ...
    if (x.children && x.children.length) {
      const childrenRoute = mergeServerRoute(x.children, sysRouteMap, getValueByKeys(r.meta, "matched", []));
      r.children = childrenRoute[0];
      routeToMeta = { ...routeToMeta, ...childrenRoute[1] };
    }
    rs.push(r);
  });
 ...
 * @param routes
 */
export const restRoute = (router: Router, routes: RouteRecordRaw[]): void => {
 ...
 * @param routes
 */
export const toValidRoute = (routes: RouteRecordRaw[]): RouteRecordRaw[] => {
  const rs: RouteRecordRaw[] = [];
  routes.forEach((x: RouteRecordRaw) => {
    if (x.meta && x.meta.isNavigationMenu !== false) {
      if (x.children && x.children.length) {
        x.children = toValidRoute(x.children);
      }
      rs.push(x);
    }
  });
...
/**
 * 注册路由，keep-alive不支持多级路由，此处转化为1级路由
 * @param router
 * @param rs
 */
